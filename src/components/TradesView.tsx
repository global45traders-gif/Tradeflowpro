  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-100">Trade Journal Log</h1>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-800 bg-slate-950 pl-10 pr-3.5 py-2 text-xs font-medium text-slate-200 placeholder-slate-600 focus:border-emerald-500 focus:outline-none transition-colors"
            />
          </div>

          {/* Trade Type Filter */}
          <div className="flex items-center space-x-2">
            <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider hidden sm:block">Type:</span>
            <select
              value={typeFilter}
              onChange={(e) => setTypeFilter(e.target.value as any)}
              className="w-full rounded-xl border border-slate-800 bg-slate-950 px-3.5 py-2 text-xs font-medium text-slate-200 focus:border-emerald-500 focus:outline-none transition-colors"
            >
          ...
          {/* Emotion Filter */}
          <div className="flex items-center space-x-2">
            <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider hidden sm:block">Emotion:</span>
            <select
              value={emotionFilter}
          ...
          {/* Outcome Filter */}
          <div className="flex items-center space-x-2">
            <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider hidden sm:block">Outcome:</span>
            <select
              value={outcomeFilter}
          ...
          {/* Sorting Field */}
          <div className="flex items-center space-x-2">
            <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider hidden sm:block">Sort:</span>
            <select
              value={sortBy}
        ...
        {(searchTerm || typeFilter !== 'ALL' || emotionFilter !== 'ALL' || outcomeFilter !== 'ALL' || sortBy !== 'date-desc') && (
          <div className="flex items-center justify-between border-t border-slate-800/60 pt-3">
            <div className="flex items-center space-x-2 text-[11px] text-slate-400">
              ...
              <span>Showing <strong className="text-emerald-400">{filteredTrades.length}</strong> of {trades.length} entries</span>
            </div>
            <button
              onClick={resetFilters}
              className="flex items-center space-x-1 rounded bg-slate-800 px-2.5 py-1 text-[11px] font-bold text-slate-300 hover:bg-slate-700 hover:text-white transition-colors"
            >
      ...
      {filteredTrades.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-800 bg-slate-950/40 py-16 px-4 text-center">
          <AlertCircle className="h-10 w-10 text-slate-600 mb-2 animate-bounce" />
          <p className="text-sm font-semibold text-slate-400">No trading logs matched your criteria</p>
          <p className="text-xs text-slate-500 mt-1 mb-4">Try adjusting your filters, clearing your search, or add a new trade setup.</p>
          <button
            onClick={resetFilters}
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-800/40 text-xs">
              {filteredTrades.map((trade, idx) => {
                ...
                const rowBg = idx % 2 === 0 ? 'bg-slate-900/40' : 'bg-slate-950/20';

                return (
                  <tr key={trade.id} className={`${rowBg} hover:bg-slate-800/20 transition-all duration-150 group`}>
                    {/* Date */}
                    <td className="px-5 py-3.5 font-mono text-slate-400">{trade.date}</td>

                    {/* Symbol */}
                    <td className="px-5 py-3.5">
                      <div className="flex flex-col">
                        <span className="font-semibold text-slate-100 tracking-tight">{trade.symbol}</span>
                        {trade.notes && (
                          <span className="text-[10px] text-slate-500 max-w-[200px] truncate" title={trade.notes}>
                            {trade.notes}
                          </span>
                        )}
                      </div>
                    ...
                    {/* Quantity */}
                    <td className="px-5 py-3.5 text-right font-mono text-slate-300">{trade.quantity}</td>
                    ...
                    <td className={`px-5 py-3.5 text-right font-mono font-bold text-sm ${
                      isBreakEven ? 'text-slate-400' : isProfit ? 'text-emerald-400' : 'text-red-400'
                    }`}>
                    ...
                    <td className="px-5 py-3.5 text-center font-mono font-semibold text-slate-300">
                      {trade.rrRatio !== undefined && !isNaN(trade.rrRatio) && trade.rrRatio !== 0 ? (
                        <span className={`inline-flex rounded px-1.5 py-0.5 text-[10px] ${
                          trade.rrRatio >= 2.0 ? 'bg-emerald-500/10 text-emerald-400' :
                          trade.rrRatio >= 1.0 ? 'bg-amber-500/10 text-amber-400' :
                    ...
                    <td className="px-5 py-3.5 text-center">
                      <span className={`inline-block rounded-full px-2.5 py-0.5 text-[10px] font-medium ${
                        trade.emotion === 'Confidence' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' :
                        trade.emotion === 'Patience' ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' :
                        trade.emotion === 'FOMO' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' :
                        trade.emotion === 'Fear' ? 'bg-red-500/10 text-red-400 border border-red-500/20' :
                        trade.emotion === 'Revenge' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' :
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <div className="flex items-center justify-end space-x-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                        <button
                          ...
                          className="rounded p-1 text-slate-400 hover:bg-slate-800 hover:text-emerald-400 transition-colors"
                        >
